...
### Set number of simulations and burnin. If not set, we have defult to 10^5 iterations and 10^4 burnin

ifelse(is.na(args[5]),nsim<-10^5,nsim<-as.numeric(args[5]))
ifelse(is.na(args[6]),nburnin<-10^4,nburnin<-as.numeric(args[6]))
cat("Running with",nburnin,"warmup rounds and",nsim,"iterations\n")

#########################
#Trying to generalize the number of conditions
#########################
